...
public class BrowseSetMealPage implements iPage{
    ...
    public BrowseSetMealPage(Session s, Order order){
        this.session = s;
    ...
    public void viewOptions(){
        int numOptions = 0;
        System.out.println("Select the Set meal you would like: ");
        ...
        for(MenuItem m : this.session.getAllMenuItems()){
            ...
            if(m.getCategory() == MealCategory.SETMEAL && m.getBranch().getBranchName().equals(currentBranchName)){
                numOptions++;
        ...
        if(numOptions == 0) {
            System.out.println("No set meals available.");
        }
